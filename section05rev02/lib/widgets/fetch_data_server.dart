...
import 'package:section05rev02/model/product_model.dart';

class FetchDataFromServerScreen extends StatefulWidget {
  FetchDataFromServerScreen({Key? key}) : super(key: key);

  @override
  State<FetchDataFromServerScreen> createState() =>
      _FetchDataFromServerScreenState();
}

class _FetchDataFromServerScreenState extends State<FetchDataFromServerScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("اطلاعات جیسون اینترنتی"),
      ),
      body: Container(
          child: _productList.isEmpty
              ...
              : Container(
                  ...
                  child: ListView(
                    children: _productList.asMap().entries.map((e) {
                      return Container(
                        height: 100,
  ...
  void _parseJson() {
    ...
    ProductParser parser = ProductParser.fromJson(jsonDecode(json));
    _productList = parser.product;
    setState(() {});
  }
}
